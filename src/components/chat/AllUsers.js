...
export default function AllUsers({
  ...
  useEffect(() => {
    const safeRooms = Array.isArray(chatRooms) ? chatRooms : [];
    const Ids = safeRooms.map((chatRoom) => {
      const members = Array.isArray(chatRoom?.members) ? chatRoom.members : [];
      return members.find((member) => member !== currentUser?.uid);
    }).filter(Boolean);
    setContactIds(Ids);
  ...
  useEffect(() => {
    ...
    setNonContacts(
      safeUsers.filter((f) => f?.uid && f.uid !== uid && !contactIds.includes(f.uid))
    );
  ...
  return (
    <>
      <ul className="overflow-auto h-[30rem]">
        ...
        <li>
          {(Array.isArray(nonContacts) ? nonContacts : []).map((nonContact, index) => (
            <div
              key={index}
